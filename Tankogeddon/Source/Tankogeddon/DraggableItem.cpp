	...
	UDragDropOperation* InOperation)
{
	...
	if(VerticalBox && TankOperation && TankOperation->DefaultDragVisual && this != TankOperation->OriginWidget)
	{
		...
		if(OverIndex >= 0)
		{
			...
			for(int32 i = 0; i < Children.Num(); ++i)
			{
				...
				if(Children[i] == this)
				{
					VerticalBox->AddChildToVerticalBox(TankOperation->OriginWidget)->SetHorizontalAlignment(EHorizontalAlignment::HAlign_Center);
				}
			}
			return true;
		}
